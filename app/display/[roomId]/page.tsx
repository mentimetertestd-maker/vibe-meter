...
export default function DisplayPage() {
  ...
  const getPosition = (index: number, text: string) => {
    ...
    return { top: `${sector.top + ((hash % 5) - 2)}%`, left: `${sector.left + (((hash * 3) % 5) - 2)}%` };
  };

  const colorList = ["text-violet-400", "text-fuchsia-400", "text-pink-400", "text-cyan-400", "text-indigo-400", "text-rose-400", "text-amber-400", "text-emerald-400"];
    ...
    <div className={`flex flex-col h-screen font-sans overflow-hidden transition-colors duration-500 ${isLight ? 'bg-white text-slate-900' : 'bg-black text-white'}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center relative overflow-hidden">
        <div className="absolute top-6 z-20 max-w-4xl px-4 pointer-events-none">
          ...
          <h1 className={`text-3xl md:text-5xl font-black tracking-tight ${isLight ? 'text-slate-900' : 'text-white'}`}>{currentQ.title}</h1>
        </div>
