...
export default function JoinPage() {
  ...
  if (!activeQuestion) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center font-sans">
        <div className="text-xl font-black text-violet-600 mb-8 tracking-tight">Isaiah6tyOne</div>
        <div className="w-10 h-10 border-4 border-violet-200 border-t-violet-600 rounded-full animate-spin mb-6"></div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 flex flex-col items-center font-sans">
      <div className="text-xl font-black text-violet-600 mb-6 tracking-tight mt-2">Isaiah6tyOne</div>
      
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 border border-slate-100">
        <div className="text-xs font-extrabold text-violet-600 mb-4 bg-violet-50 w-fit px-3 py-1 rounded-full uppercase tracking-wider">
          {activeQuestion.type === 'word_cloud' ? '☁️ 단어구름' : activeQuestion.type === 'multiple_choice' ? '📊 객관식' : '💬 익명 Q&A'}
        </div>
        <h1 className="text-2xl font-black text-slate-900 mb-8 leading-snug">{activeQuestion.title}</h1>
        
        {submitted ? (
          <div className="text-center py-10 bg-violet-50 rounded-2xl border border-violet-100">
            <div className="text-4xl mb-3">🎉</div>
            <h3 className="text-lg font-bold text-violet-900 mb-1">답변이 제출되었습니다!</h3>
            <p className="text-slate-500 text-xs">다음 질문을 기다려주세요.</p>
          </div>
        ) : (
            ...
            {activeQuestion.type === 'multiple_choice' ? (
              <div className="space-y-3">
                {activeQuestion.options?.map((opt: string, idx: number) => (
                  <button 
                    ...
                    className={`w-full p-4 text-left border-2 rounded-2xl font-semibold transition text-base ${
                      ...
                      : 'border-slate-200 hover:border-violet-300 text-slate-700 bg-white'
                    }`}
                  >
                    {opt}
                  </button>
                ...
                <button 
                  ...
                  className="w-full bg-violet-600 text-white font-bold py-4 rounded-2xl hover:bg-violet-700 transition mt-4 shadow-lg shadow-violet-200 text-base"
                >
                  선택 완료 제출하기
                </button>
              </div>
              ...
              <div>
                <input 
                  type="text" 
                  className="w-full border-2 border-slate-200 p-4 rounded-2xl mb-4 focus:outline-none focus:border-violet-600 text-slate-900 text-base bg-slate-50 focus:bg-white transition" 
                ...
                <button 
                  ...
                  className="w-full bg-violet-600 text-white font-bold py-4 rounded-2xl hover:bg-violet-700 transition shadow-lg shadow-violet-200 text-base"
                >
                  제출하기
                </button>
              </div>
